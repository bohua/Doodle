...
(function ($) {
	$.widget("metro.datepicker", {
		...
		_createCalendar: function (to, curDate) {
			var _calendar, startMode, that = this;
			...
			if(that.options.selectMode === 'month'){
				startMode = 'month';
			}else{
				startMode = 'day';
			}
			...
			_calendar.calendar({
				multiSelect: false,
				startMode: startMode,
				selectMode: that.options.selectMode,
				...
				click: function (d, d0) {
					if (d0 !== -1) {
						...
						_calendar.calendar('setDate', d);
						to.children("input[type=text]").val(d);
						that.options.selected(d);
						that._hide();
					}
				}
			});
